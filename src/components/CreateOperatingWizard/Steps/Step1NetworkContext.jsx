...
const Step1NetworkContext = ({ data, updateData }) => {
    ...
    // ... (rest of helpers remain the same until the return statement)
    ...
    return (
        <Box sx={{ display: 'flex', gap: 3, height: '100%', overflowY: 'auto', p: 1 }}> {/* Reduced gap from 4 to 3 */}
            {/* Left Column */}
            <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2 }}> {/* Reduced gap from 4 to 2 */}

                {/* ... (Markets, Cities, Depots sections remain unchanged) ... */}
            ...
            {/* Right Column */}
            <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 2 }}> {/* Reduced gap from 3 to 2 */}
                    ...
                    <Box sx={{ display: 'flex', gap: 2, mb: 3 }}>
                        <Box sx={{ flex: 1 }}>
                            ...
                            <TextField
                                ...
                                InputProps={{
                                    endAdornment: (
                                        <InputAdornment position="end" sx={{ cursor: 'pointer' }} onClick={() => startDateRef.current?.showPicker()}>
                                            <CalendarTodayIcon sx={{ color: '#1B3E38', fontSize: 22 }} />
                                        </InputAdornment>
                                    )
                                }}
                                sx={{
                                    ...
                                    '& .MuiOutlinedInput-root': {
                                        borderRadius: 1,
                                        '& fieldset': { borderColor: '#1B3E38' }, // Green border
                                        '&:hover fieldset': { borderColor: '#1B3E38' },
                        ...
                        <Box sx={{ flex: 1 }}>
                            ...
                            <TextField
                                ...
                                InputProps={{
                                    endAdornment: (
                                        <InputAdornment position="end" sx={{ cursor: 'pointer' }} onClick={() => endDateRef.current?.showPicker()}>
                                            <CalendarTodayIcon sx={{ color: '#1B3E38', fontSize: 22 }} />
                                        </InputAdornment>
                                    )
                                }}
